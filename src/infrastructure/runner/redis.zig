...
/// Raw posix write loop — avoids threading io through every inner helper.
...
/// An std.Io.Reader backed by a raw posix fd.
///
/// Decision: avoid std.Io.File.reader(io, &buf) which routes through
/// io.vtable.operate() — that vtable is not initialized in the test runner
/// context, causing SIGABRT. Direct posix.read bypasses the vtable entirely
/// and works identically in both production and test contexts.
const FdReader = struct {
    fd: std.posix.fd_t,
    interface: std.Io.Reader,

    const vtable: std.Io.Reader.VTable = .{ .stream = fdStream };

    pub fn init(fd: std.posix.fd_t, buf: []u8) FdReader {
        return .{
            .fd = fd,
            .interface = .{
                .vtable = &vtable,
    ...
    fn fdStream(io_reader: *std.Io.Reader, w: *std.Io.Writer, limit: std.Io.Limit) std.Io.Reader.StreamError!usize {
        const self: *FdReader = @alignCast(@fieldParentPtr("interface", io_reader));
...
fn send_command(allocator: std.mem.Allocator, fd: std.posix.fd_t, items: []const []const u8) !void {
    var aw: std.Io.Writer.Allocating = .init(allocator);
    defer aw.deinit();
    try resp.encode_array(&aw.writer, items);
    try posix_write_all(fd, aw.writer.buffered());
}

fn read_reply(allocator: std.mem.Allocator, reader: *std.Io.Reader) !bool {
...
pub fn execute(
    ...
    const addr = std.Io.net.IpAddress.parseIp4(dsn.host, dsn.port) catch |err| {
        std.log.warn("redis runner: address parse failed: url={s} err={any}", .{ redact_url(payload.url), err });
        return failure;
    };
    ...
    var fd_reader = FdReader.init(fd, &read_buf);
    const reader: *std.Io.Reader = &fd_reader.interface;

    if (dsn.password) |pass| {
        if (dsn.user) |user| {
            const items = [_][]const u8{ "AUTH", user, pass };
            send_command(allocator, fd, &items) catch |err| {
    ...
    send_command(allocator, fd, &cmd_items) catch |err| {
...
fn redis_peer_stub(io: std.Io, server: *std.Io.net.Server, reply: []const u8) void {
    const stream = server.accept(io) catch return;
    defer stream.close(io);
    var discard: [4096]u8 = undefined;
    _ = std.posix.read(stream.socket.handle, &discard) catch {};
    posix_write_all(stream.socket.handle, reply) catch {};
}
...
fn redis_multi_stub_2(io: std.Io, server: *std.Io.net.Server, r1: []const u8, r2: []const u8) void {
    const stream = server.accept(io) catch return;
    defer stream.close(io);
    const fd = stream.socket.handle;
    var buf: [4096]u8 = undefined;
    _ = std.posix.read(fd, &buf) catch return;
    posix_write_all(fd, r1) catch return;
    _ = std.posix.read(fd, &buf) catch return;
    posix_write_all(fd, r2) catch return;
}

fn redis_multi_stub_3(io: std.Io, server: *std.Io.net.Server, r1: []const u8, r2: []const u8, r3: []const u8) void {
    const stream = server.accept(io) catch return;
    defer stream.close(io);
    const fd = stream.socket.handle;
    var buf: [4096]u8 = undefined;
    ...
    posix_write_all(fd, r2) catch return;
    _ = std.posix.read(fd, &buf) catch return;
    posix_write_all(fd, r3) catch return;
}
...
test "execute with RPUSH command pushes job identifier to redis list" {
    ...
    const t = try std.Thread.spawn(.{}, redis_peer_stub, .{ io, &server, ":1\r\n" });
...
test "execute returns success=false when server replies with RESP error on queue command" {
    ...
    const t = try std.Thread.spawn(.{}, redis_peer_stub, .{ io, &server, "-WRONGTYPE Operation against a key holding the wrong kind of value\r\n" });
...
test "execute with two-arg AUTH sends credentials and command successfully" {
    ...
    const t = try std.Thread.spawn(.{}, redis_multi_stub_2, .{ io, &server, "+OK\r\n", ":1\r\n" });
...
test "execute returns success=false when AUTH is rejected by server" {
    ...
    const t = try std.Thread.spawn(.{}, redis_peer_stub, .{ io, &server, "-WRONGPASS invalid username-password pair or user is disabled.\r\n" });
...
test "execute returns success=false when SELECT is rejected by server" {
    ...
    const t = try std.Thread.spawn(.{}, redis_peer_stub, .{ io, &server, "-ERR DB index is out of range\r\n" });
...
test "execute with credentials and non-zero db sends AUTH then SELECT then command" {
    ...
    const t = try std.Thread.spawn(.{}, redis_multi_stub_3, .{ io, &server, "+OK\r\n", "+OK\r\n", ":1\r\n" });
...
test "redis runner publishes job identifier to channel" {
    ...
    const io = std.testing.io;
    var address = try std.Io.net.IpAddress.parseIp4("127.0.0.1", 6379);
...
test "redis runner RPUSH round-trip confirms job identifier at list tail via LRANGE" {
    ...
    var cmd_aw: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer cmd_aw.deinit();
    const lrange_items = [_][]const u8{ "LRANGE", list_key, "0", "-1" };
    try resp.encode_array(&cmd_aw.writer, &lrange_items);
    try posix_write_all(stream.socket.handle, cmd_aw.writer.buffered());

    var net_read_buf: [4096]u8 = undefined;
    var fd_rdr = FdReader.init(stream.socket.handle, &net_read_buf);
    const net_reader: *std.Io.Reader = &fd_rdr.interface;
