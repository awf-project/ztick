...
pub fn run(allocator: std.mem.Allocator, argv: []const []const u8, identifier: u128) execution.Response {
...
test "subprocess run returns success for zero exit code" {
    ...
    const response = run(std.testing.allocator, &argv, 100);
...
test "subprocess run returns failure for non-zero exit code" {
    ...
    const response = run(std.testing.allocator, &argv, 200);
...
test "subprocess run returns failure for nonexistent binary" {
    ...
    const response = run(std.testing.allocator, &argv, 300);
...
test "subprocess run preserves identifier in response" {
    const argv = [_][]const u8{"/bin/true"};
    const response = run(std.testing.allocator, &argv, 0xdeadbeef_deadbeef);
